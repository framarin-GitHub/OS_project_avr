...
serialSetInterfaceAttribs uses termios.h struct to set serial port options, namely
  speed, parity check, size of bit communication (8-bit). It also allows to set blocking read and
  write communication. VMIN sets the number of char to read to should_block, as a consequence it blocks until
  one char is read or not blocking with 0. VTIME sets the timeout for read at 0.5 sec.
