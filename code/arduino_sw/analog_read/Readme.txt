...
we switch on conversion by setting ADCSRA's bit ADSC==1. We busy wait for it to be ADSC==0, meaning conversion is over.
we return ADC which is the concat of ADCL and ADCH, with max precision of 10 bit. In other words, we have a sample ranging from 0 (0V) to 1023(5V).


void initADC(uint8_t pin);

initADC uses 3 registers: 1)ADMUX;2)ADCSRA3)ADCSRB.
1)
